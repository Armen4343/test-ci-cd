...
//                $salePrice = $totalPrice;
                ...
                $name = $mainitem->name;
                $promoDays = $mainitem->promo_days;
                $timeRange = $mainitem->time_range;
                $dateRange = $mainitem->date_range;
                ...
                $strExpiryDate = "";
                if($mainitem->expire_date!="")
                {
                    $strExpiryDate = date("d-m-Y", strtotime($mainitem->expire_date));
                }
                ...
                foreach($menuitems as $item)
                {
                    ...
                    if($strExpiryDate=="" && $item->expire_date!="")
                    {
                        $strExpiryDate = $item->expire_date;
                    }
                    else {
                        if($item->expire_date!="" && strtotime($item->expire_date)<strtotime($strExpiryDate))
                        {
                            $strExpiryDate = $item->expire_date;
                        }
                    }
                ...
                if($strExpiryDate!="")
                {
                    $strExpiryDate = date("d-m-Y", strtotime($strExpiryDate));
                }
...
//                    $salePrice = $totalPrice;
    ...
    public function singleitems(Request $request){
        ...
        $menus = Menu::where('vendor_id', $id)
            ...
            ->where(DB::raw("if(menus.expire_date is null, CURDATE(), date_format(str_to_date(menus.expire_date, '%Y-%m-%d'), '%Y-%m-%d') )"), ">=", date("Y-m-d"))
            ->where(DB::raw("if(menus.date_range is null, CURDATE(), date_format(str_to_date(SUBSTRING_INDEX(menus.date_range, ' - ', 1), '%m/%d/%Y'), '%Y-%m-%d') )"), "<=", date("Y-m-d"))
            ->where(DB::raw("if(menus.date_range is null, CURDATE(), date_format(str_to_date(SUBSTRING_INDEX(menus.date_range, ' - ', -1), '%m/%d/%Y'), '%Y-%m-%d') )"), ">=", date("Y-m-d"))
            ->where('menus.status', $nMenuPublish)
            ->get();
        ...
        foreach($menus as $menu){
            ...
            if(!isset($menu_array[$strKey]['expiry_date']))
            {
                ...
                if($menu->expire_date!="")
                {
                    $strExpireDate = $menu->expire_date;
                }
                $menu_array[$strKey]['expiry_date'] = $strExpireDate;
            }
            else {
                if($menu->expire_date!="")
                {
                    if($menu_array[$strKey]['expiry_date']=="")
                    {
   ...
   return view('front-end.cat_singlepage_new',compact('items','groupedItems','vendor','cuisines','menus','menu_array','allitems','ratings','orders'));
    ...
    public function search_vendor(Request $request) {
		...
		$restaurants = User::
            ...
            $restaurants = User::where('city', 'LIKE', '%' . $location . '%')->where('disable_restaurant', 'no')->where('status', 'active')->get();
        }
