		...
		<form action="{{route('categories.update',$data->id)}}" method="post" enctype="multipart/form-data">
			...
			<div class="row">
			 <div class="col-md-6 my-3">
				 ...
				 <input type="text" class="form-control" name="title" placeholder="Enter Category Title" value="{{$data->title}}">
			 </div>
			 <div class="col-md-6 my-3">
				 <label class="fw-bold fs-6 mb-2">Category Image(500*500)</label>
			...
			<div class="col-md-6 my-3">
				 <img src="{{asset($data->image)}}" width="200px" class="rounded-1"/>
			 </div>
				<div class="col-md-6 my-3">
				<label class="fw-bold fs-6 mb-2">Category Short Description</label>
				<textarea class="form-control" placeholder="Enter Category Short Description" name="short_description">{{$data->description}}</textarea>
			 </div>
			</div>
