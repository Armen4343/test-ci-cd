<x-jet-form-section submit="updateProfileInformation">
    ...
    <x-slot name="form">
        ...
        <div class="col-span-6 sm:col-span-4">
            ...
            @if (Laravel\Fortify\Features::enabled(Laravel\Fortify\Features::emailVerification()) && ! $this->user->hasVerifiedEmail())
                <p class="text-sm mt-2">
                    {{ __('Email non verificata. ') }}

                    <button type="button" class="underline text-sm text-gray-600 hover:text-gray-900" wire:click.prevent="sendEmailVerification">
                        {{ __('Clicca per inviare una nuova email di verifica') }}
                    </button>
                </p>

                @if ($this->verificationLinkSent)
                    <p v-show="verificationLinkSent" class="mt-2 font-medium text-sm text-green-600">
                        {{ __('Nuova mail inviata') }}
                    </p>
                @endif
            @endif
        </div>
		...
		<!-- Zipcode -->
        <div class="col-span-6 sm:col-span-4">
            <x-jet-label for="zipcode" value="{{ __('Codice Postale') }}" />
        ...
        <div class="col-span-6 sm:col-span-4">
            <x-jet-label for="business_description" value="{{ __('Descrizione ') }}" />
        ...
        <div class="col-span-6 sm:col-span-4">
			...
			<div class="col-span-6 sm:col-span-4">
				<x-jet-label for="vendor_payment_password" value="{{ __('Password Finanziaria') }}" />
  ...
  function checkPhone(phone)
		{
			   var PATTERN = "[+]{1}[0-9]{12}";
				...
				document.getElementById('phone-error').innerHTML = "Inserisci in questo formato ( e.g: +390000000000 )";
	 ...
	 function checkBusinessDescription(text)
		{
				...
				document.getElementById('business-description-error').innerHTML = "Descrizione dell'attivita' deve essere meno di 100 caratteri!";
					document.getElementById("btn-submit").disabled = true;
					return (false)
	 ...
	 function checkaddress(text)
		{
				...
				if (text.length <= 0 || text.length > 100) {
				document.getElementById('address-error').innerHTML = "Indirizzo deve essere meno di 100 caratteri";
					document.getElementById("btn-submit").disabled = true;
					return (false)
	 ...
	 function checkZipCode(text)
		{
				...
				if (text.length <= 0) {
				document.getElementById('zipcode-error').innerHTML = "Inserisci Codice Postale";
					document.getElementById("btn-submit").disabled = true;
					return (false)
